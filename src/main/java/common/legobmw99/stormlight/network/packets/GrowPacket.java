		...
		public IMessage onMessage(final GrowPacket message, final MessageContext ctx) {
	        ...
	        mainThread.addScheduledTask(new Runnable() {
	            ...
	            public void run() {
	            	...
	            	IBlockState ibs = ctx.getServerHandler().player.world.getBlockState(bp);
		            IGrowable igrowable = (IGrowable)ibs.getBlock();

	            	 if (igrowable.canUseBonemeal(ctx.getServerHandler().player.world, ctx.getServerHandler().player.world.rand, bp, ibs))
	                    {
	                        igrowable.grow(ctx.getServerHandler().player.world, ctx.getServerHandler().player.world.rand, bp, ibs);
	                    }     
	        		}
	        });		return null;
		}
