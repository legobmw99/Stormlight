	...
	public static class Handler implements IMessageHandler<TransformBlockPacket, IMessage>{
		...
		public IMessage onMessage(final TransformBlockPacket message, final MessageContext ctx) {
	        ...
	        mainThread.addScheduledTask(new Runnable() {
	            @Override
	            public void run() {
	            	ctx.getServerHandler().playerEntity.worldObj.setBlockState(new BlockPos(message.x,message.y,message.z), (IBlockState) (Block.getBlockById(message.blockID).getBlockState().getBaseState()));
	        		}
	        });		return null;
		}
	}
